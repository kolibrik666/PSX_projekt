...
public class Test : MonoBehaviour
{
    ...
    [SerializeField] private float jumpBobSpeed = 12f;
    [SerializeField] private float jumpBobAmount = 0.04f;
    ...
    private void HandleHeadBobJump()
    {
        if (ShouldJump)
        {
            ...
            {
                Debug.Log("jump");
                timer += Time.deltaTime * (jumpBobSpeed);
                playerCamera.transform.localPosition = new Vector3(
                 playerCamera.transform.localPosition.x,
                 defaultYPos + Mathf.Sin(timer) * (jumpBobAmount),
                 playerCamera.transform.localPosition.z);
            }
        }
